...
from dataset.dataset_scanning2 import ScanningDataset2
...
def prepare_batch(target, bg_type="black"):
    ...
    if bg_type == "checker":
        background = torch.tensor(util.checkerboard(target["img"].shape[1:3], 8), dtype=torch.float32, device="cuda")[
            None, ...
        ]
    elif bg_type == "black":
...
def initial_guess_material(geometry, mlp, FLAGS, init_mat=None):
    ...
    if mlp:
        ...
        if FLAGS.random_textures:  # Promjena prije bilo: or init_mat is None
            ...
            kd_init = (torch.ones(size=FLAGS.texture_res + [num_channels], device="cuda") / 2) * (kd_max - kd_min)[
                None, None, 0:num_channels
...
def optimize_mesh(
    ...
    dataloader_train = torch.utils.data.DataLoader(
        dataset_train, batch_size=FLAGS.batch, collate_fn=dataset_train.collate, shuffle=True, num_workers=4
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-mtl", "--mtl-override", type=str, default=None)

    FLAGS = parser.parse_args()

    # FLAGS.mtl_override        = "./out/nike_crossfit_base_mesh_2_passes_no_clamp/mesh_mlp/mesh.mtl"                     # Override material of model
    ...
    FLAGS.nrm_min = [-1.0, -1.0, -1.0]  # Limits for normal map
    ...
    # ==============================================================================================
    # dataset_train = ScanningDataset2(FLAGS.ref_mesh, FLAGS, examples=(FLAGS.iter + 1) * FLAGS.batch)
    # dataset_validate = ScanningDataset2(FLAGS.ref_mesh, FLAGS)
    ...
    init_mat = {
        "normal": base_mesh.material["normal"],
        "ks": base_mesh.material["ks"],
        "kd": base_mesh.material["kd"],
        "bsdf": base_mesh.material["bsdf"]
    }
    mat = initial_guess_material(geometry, True, FLAGS, init_mat=init_mat)
    ...
    #     FLAGS,
    #     pass_idx=0,
    #     pass_name="mesh_mlp_pass",
    #     warmup_iter=100,
    #     optimize_light=not FLAGS.lock_light,
    #     optimize_geometry=not FLAGS.lock_pos,
    # )

    # mask, kd, ks, normal = render.render_uv(glctx, base_mesh, FLAGS.texture_res, mat["kd_ks_normal"])

    if FLAGS.mtl_override:
        nrm_min, nrm_max = torch.tensor(FLAGS.nrm_min, dtype=torch.float32, device="cuda"), torch.tensor(
            FLAGS.nrm_max, dtype=torch.float32, device="cuda"
        )
        ks_min, ks_max = torch.tensor(FLAGS.ks_min, dtype=torch.float32, device="cuda"), torch.tensor(
        ...
        normal = texture.create_trainable(
            init_mat["normal"], FLAGS.texture_res, not FLAGS.custom_mip, [nrm_min, nrm_max]
        )
        print(len(init_mat["ks"].data.shape))
        ksR = np.random.uniform(size=FLAGS.texture_res + [1], low=0.0, high=0.01)
        ksG = np.random.uniform(size=FLAGS.texture_res + [1], low=ks_min[1].cpu(), high=ks_max[1].cpu())
        ksB = np.random.uniform(size=FLAGS.texture_res + [1], low=ks_min[2].cpu(), high=ks_max[2].cpu())

        ks = texture.create_trainable(np.concatenate((ksR, ksG, ksB), axis=2), FLAGS.texture_res, not FLAGS.custom_mip, [ks_min, ks_max])
        print("AAAA")
        # ks = texture.create_trainable(init_mat["ks"], FLAGS.texture_res, not FLAGS.custom_mip, [ks_min, ks_max])
        # print("BBB")

        kd = texture.create_trainable(init_mat["kd"], FLAGS.texture_res, not FLAGS.custom_mip, [kd_min, kd_max])
        print(ks.data)
        print(ks.data.shape)
    ...
    base_mesh.material = material.Material(
        {
            "bsdf": init_mat["bsdf"],
            # "kd": texture.Texture2D(kd, min_max=[kd_min, kd_max]),
            # "ks": texture.Texture2D(ks, min_max=[ks_min, ks_max]),
    ...
    # ==============================================================================================
    mesh_out = "mesh"
    if FLAGS.mtl_override is None:
        mesh_out = "mesh_mlp"
    if FLAGS.local_rank == 0:
        final_mesh = geometry.getMesh(mat)
        os.makedirs(os.path.join(FLAGS.out_dir, mesh_out), exist_ok=True)
        obj.write_obj(os.path.join(FLAGS.out_dir, mesh_out), final_mesh)
        light.save_env_map(os.path.join(FLAGS.out_dir, mesh_out + "/probe.hdr"), lgt)
