...
def initial_guess_material(geometry, mlp, FLAGS, init_mat=None):
    ...
    if mlp:
        ...
        if FLAGS.random_textures: # Promjena prije bilo: or init_mat is None
            ...
            kd_init = (torch.ones(size=FLAGS.texture_res + [num_channels], device='cuda') / 2) * (kd_max - kd_min)[None, None, 0:num_channels] + kd_min[None, None, 0:num_channels]
...
if __name__ == "__main__":
    ...
    if FLAGS.base_mesh is None:
        ...
        init_mat = {'normal': base_mesh.material["normal"], "bsdf": base_mesh.material["bsdf"]}
        mat = initial_guess_material(geometry, True, FLAGS, init_mat=init_mat)
        pass_name = "mesh_mlp_pass"

        geometry, mat = optimize_mesh(glctx, geometry, mat, lgt, dataset_train, dataset_validate, FLAGS, pass_idx=0, pass_name=pass_name,
        ...
        if FLAGS.mtl_override:
            nrm_min, nrm_max = torch.tensor(FLAGS.nrm_min, dtype=torch.float32, device='cuda'), torch.tensor(FLAGS.nrm_max, dtype=torch.float32, device='cuda')
            normal = texture.create_trainable(init_mat['normal'], FLAGS.texture_res, not FLAGS.custom_mip, [nrm_min, nrm_max])
