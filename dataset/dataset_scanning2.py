import os
import glob

import torch
import numpy as np

from render import util

from .dataset import Dataset


def _load_mask(fn):
    img = torch.tensor(util.load_image(fn), dtype=torch.float32)
    if len(img.shape) == 2:
        img = img[..., None].repeat(1, 1, 3)
    return img


def _load_img(fn):
    img = util.load_image_raw(fn)
    if img.dtype != np.float32:  # LDR image
        img = torch.tensor(img / 255, dtype=torch.float32)
        img[..., 0:3] = util.srgb_to_rgb(img[..., 0:3])
    else:
        img = torch.tensor(img, dtype=torch.float32)
    return img


class ScanningDataset2(Dataset):
    def __init__(self, base_dir, FLAGS, examples=None):
        self.FLAGS = FLAGS
        self.base_dir = base_dir
        self.examples = examples

        # Enumerate all image files and get resolution
        self.all_img = [
            f
            for f in sorted(
                glob.glob(os.path.join(self.base_dir, "images", "camera_*", "*")), key=lambda x: os.path.basename(x)
            )
            if f.lower().endswith("png") or f.lower().endswith("jpg") or f.lower().endswith("jpeg")
        ]
        self.all_mask = [
            f
            for f in sorted(
                glob.glob(os.path.join(self.base_dir, "masks", "camera_*", "*")), key=lambda x: os.path.basename(x)
            )
            if f.lower().endswith("png") or f.lower().endswith("jpg") or f.lower().endswith("jpeg")
        ]
        self.resolution = _load_img(self.all_img[0]).shape[0:2]

        # Load camera poses
        poses_bounds = np.load(os.path.join(self.base_dir, "poses_bounds.npy"))
        poses = poses_bounds[:, :-2].reshape([-1, 3, 5]).transpose([1, 2, 0])
        poses = np.moveaxis(poses, -1, 0).astype(np.float32)

        lcol = np.array([0, 0, 0, 1], dtype=np.float32)[None, None, :].repeat(poses.shape[0], 0)
        self.imvs = torch.tensor(np.concatenate((poses[:, :, 0:4], lcol), axis=1), dtype=torch.float32)
        self.aspect = self.resolution[1] / self.resolution[0]  # width / height

        assert len(self.all_img) == self.imvs.shape[0] and len(self.all_mask) == self.imvs.shape[0]

        # Load projection matrices for each camera
        self.camera_projections = {}
        proj_files = sorted(glob.glob(os.path.join(self.base_dir, "intrinsics", "projection_matrix_camera_*.npy")))
        for proj_file in proj_files:
            # Extract camera ID from filename
            camera_id = int(os.path.basename(proj_file).split("_")[-1].split(".")[0])
            self.camera_projections[camera_id] = torch.from_numpy(np.load(proj_file))

        if self.FLAGS.local_rank == 0:
            print(
                "ScanningDaaset: %d images with shape [%d, %d]"
                % (len(self.all_img), self.resolution[0], self.resolution[1])
            )

        # Pre-load from disc to avoid slow png parsing
        if self.FLAGS.pre_load:
            self.preloaded_data = []
            for i in range(self.imvs.shape[0]):
                self.preloaded_data += [self._parse_frame(i)]

    def _parse_frame(self, idx):
        # Load image+mask data
        img = _load_img(self.all_img[idx])
        mask = _load_mask(self.all_mask[idx])
        img = torch.cat((img, mask[..., 0:1]), dim=-1)

        mv = torch.linalg.inv(self.imvs[idx, ...])
        campos = torch.linalg.inv(mv)[:3, 3]

        # Determine the camera ID from the image path
        camera_id = int(os.path.basename(os.path.dirname(self.all_img[idx])).split("_")[-1])
        projection_matrix = self.camera_projections[camera_id]
        mvp = projection_matrix @ mv

        return img[None, ...], mv[None, ...], mvp[None, ...], campos[None, ...]  # Add batch dimension

    def __len__(self):
        return self.imvs.shape[0] if self.examples is None else self.examples

    def __getitem__(self, itr):
        if self.FLAGS.pre_load:
            img, mv, mvp, campos = self.preloaded_data[itr % self.imvs.shape[0]]
        else:
            img, mv, mvp, campos = self._parse_frame(itr % self.imvs.shape[0])

        return {
            "mv": mv,
            "mvp": mvp,
            "campos": campos,
            "resolution": self.resolution,
            "spp": self.FLAGS.spp,
            "img": img,
        }
